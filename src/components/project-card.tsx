  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="absolute left-4 top-4 font-mono text-[0.62rem] font-semibold uppercase text-white/80">
          {project.category}
        </div>
        <div className="absolute right-4 top-4 font-mono text-[0.62rem] font-semibold text-white/76">
          {project.year}
        </div>
        ...
        <div className="absolute inset-x-0 bottom-0 translate-y-3 p-4 opacity-0 transition duration-300 group-hover:translate-y-0 group-hover:opacity-100 group-focus-visible:translate-y-0 group-focus-visible:opacity-100">
          ...
          <p className="mt-1 text-xs text-white/76">{project.subtitle}</p>
          <div className="mt-3 flex flex-wrap gap-1.5">
            {project.stack.map((item) => (
              <span
                className="rounded-[4px] border border-white/28 bg-white/14 px-2 py-1 font-mono text-[0.58rem] font-semibold uppercase text-white"
                key={item}
              >
                {item}
              </span>
